...
- [ ] **M1 - Walking skeleton.** Window opens -> spawns the login shell in a PTY ->
  shell output paints in a GPU cell grid -> keystrokes reach the shell -> clean
  quit. Single pane, one shell, ugly but real. Retires the hardest integration
  risks (GPU surface, PTY plumbing, event loop, glyph upload).
